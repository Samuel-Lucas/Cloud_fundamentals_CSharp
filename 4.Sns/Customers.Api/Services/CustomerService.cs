...
public class CustomerService : ICustomerService
{
    ...
    private readonly ISnsMessenger _sns;

    public CustomerService(ICustomerRepository customerRepository, 
        IGitHubService gitHubService, ISnsMessenger sns)
    {
    ...
    public async Task<bool> CreateAsync(Customer customer)
    {
        ...
        if (response)
        {
            await _sns.PublishMessageAsync(customer.ToCustomerCreatedMessage());
        }
    ...
    public async Task<bool> DeleteAsync(Guid id)
    {
        ...
        if (response)
        {
            await _sns.PublishMessageAsync(new CustomerDeleted()
            {
                Id = id
            });
        }
